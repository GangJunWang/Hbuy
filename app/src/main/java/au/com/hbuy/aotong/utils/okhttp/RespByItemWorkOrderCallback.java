 ...
 */
public abstract class RespByItemWorkOrderCallback<T> extends Callback<Resp<T>> {
    private Gson gson;
    private Activity mContext;

    public RespByItemWorkOrderCallback(Activity context) {
    ...
    public void onResponse(Resp<T> response, int id) {
        if (null == response) {
         ...
         try{
             onBusiness(response.time, response.pictures);
            } catch (Exception e) {
            e.printStackTrace();
            onError(e);
        }
...
//            json = m.group(1);
     ...
     * @param data 掺杂的内容
     */
    public void onBusiness(String Code, String[] data) {
        //no-op
    }
