 ...
 */
public abstract class RespSendCodeCallback<T> extends Callback<Resp<T>> {
    private Gson gson;
    private Activity mContext;
    private CustomProgressDialog mDialog;
    public RespSendCodeCallback(Activity context) {
        GsonBuilder builder = new GsonBuilder();
        gson = builder.create();
        this.mContext = context;
    }

    public RespSendCodeCallback(Activity context, CustomProgressDialog dialog) {
        GsonBuilder builder = new GsonBuilder();
        gson = builder.create();
        this.mContext = context;
        this.mDialog = dialog;
    ...
    public Resp parseNetworkResponse(Response response, int id) throws Exception {
        Resp respByOrder = null;
        KLog.d(response.headers());
        KLog.d(response.code());
    ...
    public void onResponse(Resp<T> response, int id) {
        try {
           onSuccess(response.status, response.msg);
        } catch (Exception e) {
            e.printStackTrace();
            onError(e);
        }
...
//            json = m.group(1);
     ...
     * 响应成功
     */
    public abstract void onSuccess(String str, String msg);
