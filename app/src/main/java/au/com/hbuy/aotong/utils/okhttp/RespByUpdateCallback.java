 ...
 */
public abstract class RespByUpdateCallback<T> extends Callback<RespByUpdate<T>> {
    private Gson gson;
    private Activity mContext;

    public RespByUpdateCallback(Activity context) {
    ...
    @Override
    public RespByUpdate parseNetworkResponse(Response response, int id) throws Exception {
        RespByUpdate RespByUpdate = null;
        ...
        try {
            RespByUpdate = gson.fromJson(trim(string), getType());
        } catch (Exception e) {
            // 错误信息的结构holy shit
            e.printStackTrace();
            RespByUpdate = gson.fromJson(string, new TypeToken<RespByUpdate<ErrorMsg>>() {
    ...
    @Override
    public void onResponse(RespByUpdate<T> response, int id) {
        if (null == response) {
            onBusiness();
            return;
        }
        try {
            onSuccess(response.slide, response.version);
        } catch (Exception e) {
            e.printStackTrace();
            onError(e);
        }
...
//            SToast.show(e.getMessage());
    ...
    private Type getType() {
        ...
        return $Gson$Types.newParameterizedTypeWithOwner(null, RespByUpdate.class, _type);
    }
...
//            json = m.group(1);
     ...
     * 响应成功
     */
    public abstract void onSuccess(T t, Version v);
