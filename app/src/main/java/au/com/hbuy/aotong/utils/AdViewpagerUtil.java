...
import au.com.hbuy.aotong.adapter.ImageViewPagerAdapter;
...
public class AdViewpagerUtil {
    ...
    private int delayTime = 500000; //自动轮播的时间间隔
     ...
     * @param urls
     */
    public AdViewpagerUtil(Context context, ViewPager viewPager, int[] urls) {
     ...
     * @param urls
     */
    public AdViewpagerUtil(Context context, ViewPager viewPager, LinearLayout dotlayout, int dotsize, int dotoffset, int[] urls) {
        this.context = context;
        this.viewPager = viewPager;
    ...
    private void initAdimgs(int[] urls) {
    ...
    private void setImg(int length, int[] urls) {
        if (urls.length > 0) {
            imgsize = length;
            for (int i = 0; i < length; i++) {
                if (i < length - 2) {
                    final int index = i;
                    mImageViews[i + 1].setImageResource(urls[i]);
            ...
            mImageViews[0].setImageResource(urls[urls.length - 1]);
            mImageViews[length - 1].setImageResource(urls[0]);
