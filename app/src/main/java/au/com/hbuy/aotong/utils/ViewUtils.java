...
public class ViewUtils {
	...
	public static int scale(int displayWidth, int displayHeight, float pxValue) {
		...
		float scale = 1;
		try {
			float scaleWidth = (float) displayWidth / ConfigConstants.UI_WIDTH;
			float scaleHeight = (float) displayHeight / ConfigConstants.UI_HEIGHT;
	 ...
	 * 
	 * 描述：View树递归调用做适配.
	 * KJConfig.uiWidth = 1080;
	 * KJConfig.uiHeight = 700;
    ...
    */
    public static void setViewSize(View view,int widthPixels, int heightPixels){
        ...
        if(params == null){
            LogerUtils.debug(ViewUtils.class.getName(), "setViewSize出错,如果是代码new出来的View，需要设置一个适合的LayoutParams");
            return;
        }
