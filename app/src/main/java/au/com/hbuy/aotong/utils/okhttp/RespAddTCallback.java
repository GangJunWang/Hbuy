 ...
 */
public abstract class RespAddTCallback<T> extends Callback<RespAddT<T>> {
    private Gson gson;
    private Activity mContext;
    private CustomProgressDialog mDialog;
    public RespAddTCallback(Activity context) {
        GsonBuilder builder = new GsonBuilder();
        gson = builder.create();
        this.mContext = context;
    }

    public RespAddTCallback(Activity context, CustomProgressDialog mDialog) {
        GsonBuilder builder = new GsonBuilder();
        gson = builder.create();
    ...
    public void onResponse(RespAddT<T> response, int id) {
        ...
        try {
            ...
            int status = StringUtils.toInt(code);
            switch (status) {
                // 响应成功
            ...
            //        Toast.makeText(mContext, response.toString(), Toast.LENGTH_LONG).show(); // 临时用来处理服务器端报的错
                    break;
                default:
                    onBusiness(response.status);
                    break;
            }
        } catch (Exception e) {
            e.printStackTrace();
