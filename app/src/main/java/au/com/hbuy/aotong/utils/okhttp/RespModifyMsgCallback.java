 ...
 */
public abstract class RespModifyMsgCallback extends Callback<RespModifyMsg> {
    private Gson gson;
    private Activity mContext;
    private CustomProgressDialog progressDialog;

    public RespModifyMsgCallback(Activity context) {
        GsonBuilder builder = new GsonBuilder();
        gson = builder.create();
        this.mContext = context;
    }

    public RespModifyMsgCallback(Activity context, CustomProgressDialog dialog) {
        GsonBuilder builder = new GsonBuilder();
        gson = builder.create();
        this.mContext = context;
    ...
    public void onResponse(RespModifyMsg response, int id) {
        ...
        try {
            ...
            if (StringUtils.toLong(status) > 0 || StringUtils.toLong(response.code) > 0) {
