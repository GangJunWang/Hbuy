 ...
 */
public class WaitPayItemPage extends BasePage implements RecyclerArrayAdapter.OnLoadMoreListener, SwipeRefreshLayout.OnRefreshListener {
    private TextView mHint;
    public final static int FROM_DEL = 2;
    private View mView;
    private List<BuyWaitPayBean> mOrders;
    private EasyRecyclerView mListView;
    public String DataHint = "waitpaypage";
    private PkgOrderBuyAdapter adapter;
    private Handler handler;
    private int mPage = 1, mAllPage = 1;

    public WaitPayItemPage(FragmentActivity activity) {
        super(activity);
    }
        ...
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (NetUtils.hasAvailableNet(mActivity)) {
                    ApiClient.getInstance(mActivity.getApplicationContext()).postForm(ConfigConstants.GETALLBUYLIST + "?type=2&p=1"
            ...
            public void run() {
                ...
                if (NetUtils.hasAvailableNet(mActivity)) {
                    ApiClient.getInstance(mActivity.getApplicationContext()).postForm(ConfigConstants.GETALLBUYLIST +
                            "?type=2&p=" + mPage, new RespPkgListCallback<List<BuyWaitPayBean>>(mActivity, DataHint) {
                        @Override
                        public void onFail(List<BuyWaitPayBean> lists) {
                        }
