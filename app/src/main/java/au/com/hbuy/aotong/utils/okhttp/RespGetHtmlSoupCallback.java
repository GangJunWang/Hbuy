 ...
 */
public abstract class RespGetHtmlSoupCallback<T> extends Callback<RespInform<T>> {
    private Gson gson;
    private Activity mContext;

    public RespGetHtmlSoupCallback(Activity context) {
    ...
    public RespInform parseNetworkResponse(Response response, int id) throws Exception {
        ...
        try {
          ...
          /*  Elements units = doc.getElementsByClass("unit");
            Element h1_ele = units.get(0).getElementsByTag("title").get(0);
            String title = h1_ele.text();*/

            //D:\git_open\CSDNSpider-yanzi[2014-7-28]\CSDNSplider\lib
            string = doc.title();
            KLog.d(string);
        } catch (Exception e) {
            // 错误信息的结构holy shit
    ...
    public void onResponse(RespInform<T> response, int id) {
        ...
        if (null != response && null != response.getNoCooike()) {
            onFail(null);
            return;
        }
        try {
